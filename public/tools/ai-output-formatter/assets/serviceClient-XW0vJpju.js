...
import{o as e}from"./rolldown-runtime-DAXXjFlN.js";import{n as t}from"./vendor-pdf-Dk3nZWcD.js";import{n}from"./assets-BOn1ru_N.js";function r(e){return e.replace(/\\/g,`\\textbackslash{}`).replace(/([{}$&#_%])/g,`\\$1`).replace(/~/g,`\\textasciitilde{}`).replace(/\^/g,`\\textasciicircum{}`)}function i(e){return e.replace(/[{}]/g,``)}var a=[{id:`cn-paper`,name:`中文论文`,documentClass:`ctexart`,engine:`xelatex`,packages:[`geometry`,`hyperref`,`booktabs`,`graphicx`,`amsmath`,`amssymb`,`listings`,`xcolor`,`fancyhdr`,`setspace`],preamble:`\\geometry{a4paper,top=3.5cm,bottom=3.5cm,left=2.8cm,right=2.8cm}
\\setstretch{1.5}
\\pagestyle{fancy}
\\fancyhf{}
\\fancyfoot[C]{\\thepage}
...
`;return s.file(`README.txt`,f),s.file(`compile.json`,JSON.stringify({engine:c.engine,main:`main.tex`,bibliography:n.references?.entries.length?`references.bib`:null,template:c.id,shellEscape:!1,generatedAt:new Date().toISOString()},null,2)),s.generateAsync({type:`blob`})}async function h(e,t=`cn-paper`,r){let i=await m(e,t),a=n(r||e.metadata.title||`latex-project`),o=URL.createObjectURL(i),s=document.createElement(`a`);s.href=o,s.download=`${a}.zip`,s.rel=`noopener`,document.body.appendChild(s),s.click(),s.remove(),URL.revokeObjectURL(o)}var g=`aof:document-service-url`;function _(){try{let e=localStorage.getItem(g)?.trim();if(e)return e.replace(/\/$/,``)}catch{}return`https://aof-document-converter.onrender.com`.replace(/\/$/,``)}function v(){return!!_()}async function y(e){try{let t=await e.json();return t.error||t.message||`HTTP ${e.status}`}catch{return`HTTP ${e.status}`}}async function b(){let e=_();if(!e)return{ok:!1,message:`未配置转换服务地址`};try{let t=new AbortController,n=setTimeout(()=>t.abort(),45e3),r=await fetch(`${e}/health`,{method:`GET`,signal:t.signal});return clearTimeout(n),r.ok?{ok:!0,message:`服务可用`}:{ok:!1,message:await y(r)}}catch(e){return{ok:!1,message:(e.name===`AbortError`?`连接超时`:e.message)||`网络错误`,coldStartHint:!0}}}async function x(e,t,n){let r=_();if(!r)throw Error(`转换服务未配置`);let i=new FormData;i.append(`file`,e),i.append(`inputFormat`,t.inputFormat),i.append(`outputFormat`,t.outputFormat),i.append(`standalone`,String(!!t.standalone)),i.append(`toc`,String(!!t.toc)),i.append(`numberSections`,String(!!t.numberSections)),t.pdfEngine?i.append(`pdfEngine`,t.pdfEngine):i.append(`pdfEngine`,`wkhtmltopdf`),t.highlightStyle&&i.append(`highlightStyle`,t.highlightStyle),t.wrap&&i.append(`wrap`,t.wrap),t.metadataYaml&&i.append(`metadataYaml`,t.metadataYaml),n?.referenceDocx&&i.append(`referenceDocx`,n.referenceDocx),n?.bibliography&&i.append(`bibliography`,n.bibliography),n?.csl&&i.append(`csl`,n.csl);let a={};n?.apiKey&&(a[`X-API-Key`]=n.apiKey);let o=await fetch(`${r}/v1/convert`,{method:`POST`,body:i,headers:a});if(!o.ok)throw Error(await y(o));return{jobId:(await o.json()).id}}async function S(e,t){let n=_();if(!n)throw Error(`转换服务未配置`);let r=new FormData;for(let t of e)r.append(`files`,t);r.append(`engine`,t.engine),t.mainFile&&r.append(`mainFile`,t.mainFile);let i={};t.apiKey&&(i[`X-API-Key`]=t.apiKey);let a=await fetch(`${n}/v1/latex/compile`,{method:`POST`,body:r,headers:i});if(!a.ok)throw Error(await y(a));return{jobId:(await a.json()).id}}async function C(e,t){let n=_();if(!n)throw Error(`转换服务未配置`);let r={};t&&(r[`X-API-Key`]=t);let i=await fetch(`${n}/v1/jobs/${encodeURIComponent(e)}`,{headers:r});if(!i.ok)throw Error(await y(i));return await i.json()}async function w(e,t){let n=_();if(!n)throw Error(`转换服务未配置`);let r={};t&&(r[`X-API-Key`]=t);let i=await fetch(`${n}/v1/jobs/${encodeURIComponent(e)}/download`,{headers:r});if(!i.ok)throw Error(await y(i));return i.blob()}async function T(e,t){let n=_();if(!n)return;let r={};t&&(r[`X-API-Key`]=t),await fetch(`${n}/v1/jobs/${encodeURIComponent(e)}`,{method:`DELETE`,headers:r})}var E=[{from:`markdown`,to:`docx`,needsService:!0},{from:`markdown`,to:`html`,needsService:!1},{from:`markdown`,to:`latex`,needsService:!1},{from:`markdown`,to:`pdf`,needsService:!0},{from:`docx`,to:`markdown`,needsService:!0},{from:`docx`,to:`html`,needsService:!0},{from:`html`,to:`markdown`,needsService:!1},{from:`html`,to:`docx`,needsService:!0},{from:`latex`,to:`markdown`,needsService:!0},{from:`latex`,to:`docx`,needsService:!0},{from:`latex`,to:`pdf`,needsService:!0}];export{_ as a,x as c,f as d,p as f,o as h,w as i,S as l,a as m,b as n,C as o,d as p,T as r,v as s,E as t,h as u};