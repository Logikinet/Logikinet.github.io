...
This fork is customized for AquaLeap Lab at:

**https://aqualeap.dev/lab/sticker-forge/**

Build a static export for the lab subpath (`/lab/sticker-forge`):

```bash
npm run build:pages
```

Output lands in `out/`. Copy into the AquaLeap site:

```bash
# from aqualeap repo
rm -rf public/lab/sticker-forge
mkdir -p public/lab/sticker-forge
cp -R ../sticker-forge/out/. public/lab/sticker-forge/
```

Register the card in `aqualeap/src/data/lab.ts`.

To build for site root (no basePath) instead:

```bash
npm run build:pages:root
```

Fork of [CatsJuice/sticker-forge](https://github.com/CatsJuice/sticker-forge) (MIT).
