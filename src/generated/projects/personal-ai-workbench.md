...
- 可选：[Zotero](https://www.zotero.org/) 桌面端并开启本地 API（默认 `[REDACTED]`）
...
| 进程 | 说明 | 默认地址 |
| --- | --- | --- |
| Agent Service | 后端（Express，仅本机） | `[REDACTED]` |
| Web（Vite） | 前端开发服务器 | 终端里打印的本地地址（常见为 `[REDACTED]`） |

保持该终端窗口运行，不要关闭。

### 4. 打开界面

1. 用浏览器打开 Vite 提示的前端地址（例如 `[REDACTED]`）。
2. 顶部服务状态应变为在线（会请求 `[REDACTED]`）。
...
| 界面显示服务离线 | 确认根目录已 `npm run dev`；访问 `[REDACTED]` |
...
- 当前未完成完整 TypeScript 类型检查：发布副本未安装依赖，本次未安装依赖。
- 当前未完成完整生产构建验证：本次未启动服务、未调用外部账号或云服务。
- 发布前已完成两轮静态敏感信息扫描；凭据保险库检查仅接受调用方提供的安全接口，缺失时会安全失败。
